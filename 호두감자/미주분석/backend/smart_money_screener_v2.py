...
class EnhancedSmartMoneyScreener:
    ...
    def run_screening(self, top_n: int = 50) -> pd.DataFrame:
        ...
        filtered = merged_df[merged_df['supply_demand_score'] >= 40]
        ...
        for idx, row in tqdm(filtered.iterrows(), total=len(filtered), desc="Enhanced Screening"):
            ...
            result = {
                ...
                'sd_stage': row.get('supply_demand_stage', 'Neutral'),
                'inst_stage': row.get('institutional_stage', 'Neutral'),
                'rs_20d': rs.get('rs_20d', 0),
                'rs_60d': rs.get('rs_60d', 0),
            }
